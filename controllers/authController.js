  ...
  try {
    ...
    if (user.role === "seller" && !user.isApproved) {
      return res.status(403).json({ 
        message: "Your merchant registration is currently pending platform approval." 
      });
    }
...
export const getMe = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: "Session profile no longer exists." });
    }
    res.status(200).json({ user });
  } catch (error) {
    res.status(500).json({ message: "Failed to verify session profile status." });
  }
};
...
export const register = async (req, res) => {
  ...
  try {
    ...
    if (targetRole === "buyer") {
      ...
      if (!studentRecord) {
        return res.status(400).json({ message: "Invalid student ID. Not found in university records." });
      }

      // Prevent reusable registrations
      if (studentRecord.registered) {
        return res.status(400).json({ message: "This Student ID is already linked to an account." });
      }
    ...
    const registrationMessage = targetRole === "seller" 
...
export const updateProfileAvatar = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({ message: "Upload Failed: No image file payload detected." });
    }
